  ...
  <div class="UnderIndex Bindex">
    <div class="seach_main" style="height: 0.9rem;padding-top:0.4rem">
      <div class="home_seach addressBox">
        ...
        <div style="width:6rem;margin:0 auto;" class="home_seach_input" @click="jumpTo({name:'scoendSearch',query:{type:'shop',model:'self'}})">
    ...
    <div class="mescrollBox" :style="{'bottom': $isMallShop ? '43px' : '0'}">
      <mescroll-vue ref="mescroll"  :down="mescrollDown" :up="mescrollUp" @init="mescrollInit">
        ...
        <assure-list :list="suggest" ref="recommend"></assure-list>
      </mescroll-vue>
    </div>
  </div>
</template>
...
import assureList from '../../../components/assureList'
...
export default {
  ...
  methods: {
    ...
    getBanner(){
      apiHttp.indexBanner('self').then(res=>{
        if(res.code === 0){
    ...
    getShopNav(){
      apiHttp.underShopNav('self').then(res=>{
        if(res.code === 0){
    ...
    upCallback (page, mescroll) {
      this.$http.post('/mall/second-goods/goods-list',{model:'self'}).then(res => {
