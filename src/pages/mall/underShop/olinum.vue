  ...
  <div class="UnderIndex Bindex">
    ...
    <div class="mescrollBox" :style="{'bottom': $isMallShop ? '43px' : '0'}">
      <mescroll-vue ref="mescroll"  :down="mescrollDown" :up="mescrollUp" @init="mescrollInit">
        ...
        <!-- 轮播图部分 -->
        <div id="numJ">
            <img src="../../../assets/img/oli/jife.png" alt="">
            <span>您当前的积分为：</span>
            <span>{{meoo}}</span>
        </div>
...
export default {
  ...
  data(){
    return {
      meoo:this.$store.state.user.userInfo.uid,
      swiperOption: {
  ...
  methods: {
    ...
    handChange (key){ // 点击时候的操作
      this.active = key  // 高亮
      this.suggest = []
      this.mescrollUp.page.num = 1
      this.upCallback(this.mescrollUp.page, this.mescroll)  // 调用下拉回调
    },
    getInfo () {
      apiHttp.underIndex().then(res=>{
        ...
        this.$nextTick(() => {
          this.mescroll.endSuccess(this.list1.length, false)
        })
      })
    },
    ...
    upCallback (page, mescroll) {
      this.$http.post('/mall/oto/shops-list',{lng:this.lon,lat:this.lat,disType: this.active, page: page.num}).then(res => {
        if (res.code === 0) {
          ...
          this.$nextTick(() => {
            this.mescroll.endSuccess(res.data.shopList.length, true)
          })
        }
      }).catch((e) => {
        mescroll.endErr()
