<template>
    <div class="productList">
      ...
      <List v-if="!model" :list="list" style="margin-top: 1.9rem;height: calc(100% - 105px);overflow-y: auto"></List>
      <second-list v-if="model" :list="list" style="margin-top: 1.9rem;height: calc(100% - 105px);overflow-y: auto"></second-list>
    </div>
</template>
  ...
  methods: {
    selectItem (type) {
      ...
      if(this.model){
        ...
        this.$http.post('/mall/second-goods/goods-list', obj).then(res => {
          if (res.code === 0) {
