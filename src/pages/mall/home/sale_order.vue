            ...
            <div class="content_main" v-for="(items, index) in shopList" :key="index" >
              <div class="list">
                <div class="header_main">
                  <div class="left" >
                    <img src="../../../assets/img/my/shopin.png" alt="">
                ...
                <div class="mail"  @click="handDetail(items)" >
                  <div class="left">
                    <img :src="items.goods_img || './static/img/under/111.png'" alt="">
                  </div>
                  <div class="right">
                    <div class="div1">
                      {{items.goods_name}}
                    ...
                    <div class="div3">
                      <div class="price_main"><span>¥</span>{{items.goods_price}}</div>
                    </div>
                  </div>
                </div>
                <div class="price">
                  <label>共{{items.goods_num}}件商品，合计: </label><span>¥</span>{{items.pay_price}}
                </div>
                <div class="status" v-if="items.payStatus == 0">
                  <div v-if="items.order_status == 0" class="btn1" @click="handShow(index,items.orderSn)">取消订单</div>
                  <div v-if="items.order_status == -1" class="btn" @click="handDelete(index,items.orderSn)">删除订单</div>
                  <div class="btn" v-if="items.order_status == 0" @click="jumpTo({name:'mallPayMent',query:{orderId:items.orderId}})">去支付</div>
                </div>
                <div class="status" v-if="items.shopOrderStatus == 1 && items.order_status == -3">
                  <div class="btn" @click="jumpTo({name:'mallExchange',query:{orderSn:items.orderSn,status:'-4'}})">补交资料</div>
                </div>
                <div class="status" v-if="items.order_status == -3 && items.shopOrderStatus == 0">
                  <div>等待商家审核</div>
                </div>
                <div class="status" v-if="items.payStatus == 1&&items.order_status == 0">
                  <div class="btn" @click="jumpTo({name:'mallExchange',query:{orderSn:items.orderSn}})">申请退货</div>
                </div>
                <div class="status" v-if="items.payStatus == 1&&items.order_status == 1">
...
export default {
  ...
  data () {
    return {
      tab: [{sort: 0, title: '全部'}, {sort: 1, title: '待付款'}, {sort: 2, title: '待发货'}, {sort: 3, title: '待收货'}, {sort: 4, title: '已完成'}, {sort: 5, title: '退换货'}],
  ...
  methods: {
    ...
    upCallback (page, mescroll) {
      ...
      apiHttp.sellOrderList(page.num, this.sort).then(res => {
        if (res.code === 0) {
