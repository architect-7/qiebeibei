  ...
  <div class="payMent yuWithdraw">
    ...
    <div class="content" v-if="isShow">
      ...
      <div class="payMent_list">
        ...
        <div class="list" v-for="(item,index) in withdrawType" :key="index" @click="handChange(index)" v-if="item.status">
          <div class="img_box">
...
export default {
  ...
  methods: {
    ...
    getNew () {
      apiHttp.creditNew("credit2").then(response => {
        if (response.code === 0) {
          this.isShow = true
    ...
    handSubmit1 () {
      ...
      apiHttp.creditWithdraw('save', this.price, this.username, this.mobile, this.alipay, this.active,"credit2").then(response => {
        if (response.code === 0) {
    ...
    handSubmit2 () {
      ...
      apiHttp.creditCard('save', this.price, this.username, this.mobile, this.bankCard, this.bankName, this.bankNameNo, this.active,'credit2').then(response => {
        if (response.code === 0) {
          this.$vux.toast.text('提现成功')
