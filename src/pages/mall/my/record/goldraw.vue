  ...
  <div class="payMent yuWithdraw">
    ...
    <div class="content" v-if="isShow">
      <div class="headerList">
        <div class="list">
          <div>总佣金</div>
          <div class="red">￥{{glodnum ? glodnum : 0}}</div>
        </div>
        <div class="list">
          <div>提现最小提额</div>
          <div>￥{{min}}</div>
      ...
      <div class="payMent_list">
        ...
        <div class="list" v-for="(item,index) in withdrawType" @click="handChange(index)" :key="index" v-if="item.status">
          <div class="img_box">
...
export default {
  ...
  methods: {
    ...
    getNew () {
      apiHttp.creditNew("credit3").then(response => {
        if (response.code === 0) {
          this.isShow = true
    ...
    handSubmit1 () {
      ...
      apiHttp.creditWithdraw('save', this.price, this.username, this.mobile, this.alipay, this.active,"credit3").then(response => {
        if (response.code === 0) {
          this.$vux.toast.text('提现成功')
          this.jumpTo({name: 'oligol'})
    ...
    handSubmit2 () {
      ...
      apiHttp.creditCard('save', this.price, this.username, this.mobile, this.bankCard, this.bankName, this.bankNameNo, this.active,"credit3").then(response => {
        if (response.code === 0) {
          this.$vux.toast.text('提现成功')
          this.jumpTo({name: 'oligol'})
