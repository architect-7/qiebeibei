<template>
    <div class="invite">
      ...
      <!--<div class="footer" >-->
        ...
        <p @click="onCopy(list.downloadUrl)" style="background:#01C2C7" >复制链接</p>
        <p @click="savePicture()" style="background:#01C2C7" >保存海报</p>
      </div>
      ...
      this.$http.post('/amoy/user/invite', {}, true, true).then(res => {
        if (res.code === 0) {
          ...
          res.data.img.forEach((item) => {
            this.imgs.push(this.link + '/amoy/user/invite?tpl=' + item + '&uid=' + this.info.uid + '&invite_code=' + this.info.invite_code + '&i=' + newDate)
          })
          this.data = res.data
        }
      })
