<template>
    <div class="setting">
      <x-header :left-options="{showBack: false}"><a slot="left" @click="goBack()"  class="backBox">
        ...
        设置<span class="save" style="color:#01C2C7" slot="right" @click="save()">保存</span>
      </x-header>
      <div class="photo">
        ...
        <p>ID：{{data.uid}}</p>
        <span @click = "handlePhoto(1)" style="background:#01C2C7">更换头像</span>
      </div>
      <group>
        ...
        <cell title="登录密码" value="修改" is-link link="/revise" ></cell>
        <cell title="实名认证" :value="this.data.identity_card ? '已认证' : '未认证'" is-link link="/oliname"></cell>
        <!-- <cell title="绑定微信"  is-link @click.native="bind" :value="this.data.wechat_openid ? '已绑定' : '未绑定'"></cell> -->
        <cell title="收货地址"  is-link link="/mall/addAddress" :value="this.data.wechat_openid ? '添加' : '添加'"></cell>
        <cell title="绑定支付宝"  is-link link="/add" :value="this.data.withdraw_to ? '已绑定' : '未绑定'"></cell>
       <!-- <cell title="绑定银行卡"  is-link link="/addcard" :value="this.data.bank_card ? '已绑定' : '未绑定'"></cell>
        <cell title="意见反馈"  is-link link="/oliyij"></cell>-->
      ...
      <div class="primary_btn" style="background:#01C2C7" @click="layout">退出登录</div>
...
export default {
  ...
  methods: {
    ...
    //     ...this.data
    //   }
    //   derss.address=this.addressVal
    //   this.$store.commit('setUserInfo',derss)
    // },
    handlePhoto (val) {
      this.show = true
      if (val === 1) {
  ...
  created () {
    ...
    // this.addressVal=this.$store.state.userInfo.address
  },
  activated() {
    this.getfo()
  }
