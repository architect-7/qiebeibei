                   ...
                   <div class="core">
                        <div class="core_box" v-for="(item,index) in shopList" :key="index">
                            <div class="core_t">
                                ...
                                <div style="color:#df2121">{{item.payStatus}}</div>
                            </div>
                            <div class="core_t">
                                <div>下单时间：{{item.updated_at}}</div>
                                <div>价格：￥{{item.payPrice}}</div>
                            </div>
                        </div>
                   </div>
                </div>
...
export default {
  ...
  methods: {
    ...
    upCallback (page, mescroll) {
      this.$http.post('/mall/gold/user-order-list',{page:page.num}, true, true).then((res) => {
        if (res.code === 0) {
